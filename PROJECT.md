# Current migration — September 22, 2026
...
Current gate: **release and measurement, not DNS.** Checked September 22, 2026: `mj2.pro` and `www.mj2.pro` resolve to GitHub Pages, and `mitchjmiller.com` plus `www.mitchjmiller.com` have path-preserving permanent redirects to `https://mj2.pro/`. GitHub Pages reports the custom domain and a built site, but `https_enforced` remains false while certificate provisioning is unresolved. The source and live `gh-pages` HTML contain the GA4 base loader for `G-HCKYWCZQ8E`; no verified custom CTA, form, download or tool-interaction events are implemented. Before release, review the existing agency redesign, add the privacy-respecting event map, validate locally, push the accepted source, then verify live behavior and analytics. Do not enable HTTPS enforcement until the custom certificate is valid. The M² LinkedIn page will use the new squared identity; Rio's invite requires exact profile resolution before it is sent.
