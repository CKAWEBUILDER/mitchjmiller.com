# Current migration — September 23, 2026

Mitchell approved the `mj2.pro` domain cutover, the agency redesign, the approved public/private site tree and the M² business identity. The canonical checkout is on `claude/agency-redesign` at `64a5a93`, pushed to GitHub. The redesign is built on that branch but is not the live release: GitHub Pages still serves the `gh-pages` branch (`0b9073d` remotely). The release will keep GA4 `G-HCKYWCZQ8E` for continuity, keep staging analytics-free, and keep the private portal analytics-free. See [site migration record](docs/site-migration-2026-09-21/README.md) for the approved tree, templates, redirect plan, tradeoffs and progress ledger.
