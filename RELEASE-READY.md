# RELEASE-READY — current procedure (mj2.pro, updated 2026-09-25)

Production is the GitHub Pages legacy branch deploy from the `gh-pages` root, custom domain `mj2.pro` (HTTPS enforced), built from `main`. Last release: gh-pages `94016fa` from `main` `b838034` (PROJECT.md "PUBLISHED — September 25, 2026 (00:11 EDT): posts wave 2"). The 2026-09-12 procedure further down is history; its `mitchjmiller.com` probes no longer apply (that host only forwards over HTTP).
