...
Production is the GitHub Pages legacy branch deploy from the `gh-pages` root, custom domain `mj2.pro` (HTTPS enforced), built from `main`. Last release: gh-pages `3559667` from `main` `c04e56e` (PROJECT.md "PUBLISHED — September 24, 2026 (21:55 EDT): site standards, deploy 2"). The 2026-09-12 procedure further down is history; its `mitchjmiller.com` probes no longer apply (that host only forwards over HTTP).
