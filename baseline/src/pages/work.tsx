...
export default function Work() {
  ...
  const categoryMap: Record<string, string> = {
    ...
    'sfc-surf-school': 'Personal Builds'
  };
  ...
  return (
    <Layout>
      ...
      />
      <div data-filter-group="work" data-filter-default="Core Portfolio" className="container mx-auto px-4 md:px-8 py-24">
        ...
        <div data-filter-note="Personal Builds" hidden className="mb-8 rounded-xl border border-border bg-muted/30 p-5">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredStudies.map((study) => (
            <Link key={study.slug} href={`/case-studies/${study.slug}`}
              data-filter-card={categoryMap[study.slug]} data-filter-core={coreSlugs.has(study.slug) ? "true" : "false"}
              hidden={!coreSlugs.has(study.slug)}>
