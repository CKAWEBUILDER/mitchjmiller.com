#!/usr/bin/env node
/**
 * Exporter for the LLM-ads data mandala (built on viz/directions/01-data-mandala v2).
 *
 *   node export.mjs                 # posters (1x + 2x) + verify + the GIF
 *   node export.mjs --posters       # poster-1080x1350 / 1080x1080 / 2160x2700 + the 1200x630 share card
 *   node export.mjs --gif           # anim-640x800.gif
 *   node export.mjs --verify        # type floor, ink-box overlaps, containment, crown fit
 *   node export.mjs --gif --fps 10 --colors 56 --hold 50
 *
 * The GIF is three complete renderings in a row -- each a slowed assembly with a
 * 0.70s beat between stages and a 1.50s settled hold -- followed by ONE frame held
 * for --hold seconds (default 50), then the loop repeats. Beats and holds are
 * single frames carrying a long delay, so an ~82-second loop costs ~230 frames.
 *
 ...
 * viz/kit/node_modules. Writes only into this folder; fails if the page reaches
 * the network.
 */
...
/* posters: [css width, css height, device scale, file name]. The 1200x630 frame is the post's og:image,
   so it gets a descriptive kebab-case name per docs/site-standards.md. */
const POSTERS = [[1080, 1350, 1], [1080, 1080, 1], [1080, 1350, 2],
  [1200, 630, 1, 'ads-in-free-llm-answers-data-mandala-1200x630.png']];

function report(tag, a) {
  const bad = a.under.length + a.overlaps.length + a.spill.length + a.crown.length;
  console.log(`verify ${tag}: ${a.textCount} text runs, ${a.under.length} under 14px, ${a.overlaps.length} overlaps, ` +
    `${a.spill.length} outside their box, ${a.crown.length} crown misfits`);
  ...
  a.spill.slice(0, 12).forEach(o => console.log(`   SPILL "${o.text}"  ${o.why}`));
  a.crown.slice(0, 12).forEach(o => console.log(`   CROWN "${o.text}"  ${o.why}`));
  return bad;
}
...
try {
  /* ------------------------------------------------------------- posters */
  if (wants('posters')) {
    for (const [w, h, dsf, name] of POSTERS) {
      const t = now();
      ...
      const file = join(OUT, name || `poster-${w * dsf}x${h * dsf}.png`);
      writeFileSync(file, buf);
      await page.close();
      console.log(`${name ? 'card  ' : 'poster'} ${w * dsf}x${h * dsf}  ${(statSync(file).size / 1024).toFixed(0)} KB  ${ms(t)}${name ? '  ' + name : ''}`);
    }
  }
  ...
  if (wants('verify')) {
    ...
    /* the GIF canvas is a straight scale-down of the 4:5 poster: overlaps, spills and crown fit still apply */
    const { page } = await openViz(browser, vizUrl(TEMPLATE, { size: '640x800', poster: 1 }), {
      width: 640, height: 800, deviceScaleFactor: 1,
    });
    const a = await page.evaluate(() => window.VIZ.audit());
    await page.close();
    const n = a.overlaps.length + a.spill.length;
    console.log(`verify 640x800 (gif canvas): ${a.textCount} text runs, ${a.overlaps.length} overlaps, ${a.spill.length} outside their box` +
      ` (type floor is defined at 1080 wide; normalised "under" = ${a.under.length})`);
    a.overlaps.slice(0, 12).forEach(o => console.log(`   OVERLAP "${o.a}" x "${o.b}"  ${o.ox}x${o.oy}px`));
    a.spill.slice(0, 12).forEach(o => console.log(`   SPILL "${o.text}"  ${o.why}`));
    failures += n;
  }
