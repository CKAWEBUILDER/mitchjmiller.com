...
- [x] **Activate new-domain DNS and configure legacy HTTP forwarding.** Verified Sep 23, 2026: `mj2.pro` resolves to all four GitHub Pages A records, `www.mj2.pro` CNAMEs to `ckawebuilder.github.io`, and both HTTPS hosts serve the site. `mitchjmiller.com` HTTP forwarding returns a permanent redirect to `https://mj2.pro/`, preserving the path and query string. The GitHub Pages certificate was stuck; removing/re-adding `mj2.pro` through Pages settings restarted issuance. GitHub reports the certificate approved for `mj2.pro` and `www.mj2.pro` through 2026-12-22, and HTTPS enforcement is on. Latest Pages build `1234332640` completed successfully from `gh-pages` commit `3cdf29fb0767c43ffd293f6acd5b903727c1d8da`.
...
- [ ] **Legacy HTTPS forwarding remains broken.** Verified Sep 23: `https://mitchjmiller.com/...` times out at the Namecheap URL-forwarding endpoint, while HTTP forwarding works. Namecheap documents that forwarding from HTTPS to HTTPS requires SSL on both domains; its built-in forwarding endpoint is not serving the old domain over TLS. A Cloudflare redirect could provide TLS, but the old domain currently has five Namecheap email-forwarding MX records, so preserve mail routing and other DNS records before any nameserver change. Another option is hosting with a certificate. Do not describe the old-domain migration as fully complete until a valid HTTPS request follows the permanent redirect.
- [ ] Verify GSC sitemap and GA4 realtime. DNS is live; confirm both properties and submit `https://mj2.pro/sitemap.xml`.
