...
## Cloud cross-check and Mitch's direction (05:20 EDT, Claude Code cloud session)

- **Independent check.** A cloud session redid the reconciliation from `6ad50ce` without seeing this one, before it was pushed. It picked the same 9 commits to carry over. Its release build and `main` `fd8e565`'s build (same build path, Linux Chromium) are **byte-identical, 523/523 files**; both differ from `6ad50ce` only in the one stylesheet and its 77 references. Typecheck PASS, `test-stripe.mjs` 12/12, `test-limits.mjs` 9/9. The cloud copy was discarded, not pushed.
- **`npm ci` fails on a fresh clone**: `package-lock.json` is out of sync with `package.json` (`esbuild@0.28.2` and its platform packages missing). The Mac builds work because `node_modules` already exists. Until the lock is regenerated and the release build re-proven identical, fresh clones need `npm install` and must not commit the rewritten lock.
- **The "keep mitchjmiller.com" answer is about the domain, not the folder.** Mitch's words: "idk what the dupliccate checkout is.. as far as i care we can keep mitchjmiller.com as my personal work history portfolio (might be some overlap w/ the new m2 site…)". `~/Documents/mitchjmiller.com` is a clone of this M² repository, so keeping it creates no portfolio. Still open for Mitch: (a) the domain: keep the 301 to mj2.pro and fix HTTPS, or a separate personal work-history site (its own repository and registry row); (b) archive the folder to `~/Archive/<date>/` (lossless; recommended either way).
- **Direction from Mitch, September 25 (captured, not yet a task):** he wants a defined list of services and products to optimize for; small, non-technical businesses first (an audit, then a handoff with clear instructions or an agent team that implements for a monthly fee, growing into larger clients); a large blog (data-viz interactives, fintech, glossaries) and a freemium collab lab (simulated populations, prediction models, data science, design/dev, audits, strategy consults with handoffs); an outreach idea using the SFC case for new or underperforming Waikīkī businesses; career track: forward-deployed / multi-agent engineer, applications continuing. Separate product ideas (their own projects): LabelSafe label scanner, DateNite, a happy-hour finder.
- **First decision for Mitch: name the offer** (2–4 services or products with prices). It sets the Stripe catalog (`STRIPE_PRICES`), the services page and the next content; the content lane's saturated-market draft should point at it.
